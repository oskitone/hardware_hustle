...
import { NoBreak, NoWrap } from "common/utils";
...
Hardware Hustle is a tabletop, <NoWrap>roll-and-write</NoWrap> resource management game about sustainably running an indie electronics business.
...
You have six turns to buy parts to make widgets, make widgets from parts, sell widgets for money to buy more parts, and so on.
...
The six turns are miniature spreadsheets, and the game ends when they're all filled out. The player with the most money at the end wins.
...
<p><img alt="Hardware Hustle game sheet layout" src="../screenshot.png" className={styles.screenshot} /></p>
...
The spreadsheets' rows are grouped into money <IconsParen ids={["money"]} />, parts <IconsParen ids={["A", "B", "C", "D"]} />, widgets <IconsParen ids={["AB", "ABC", "ABCD"]} />, and opportunity <IconsParen ids={["opportunity"]} />.

And their five columns are phases that you fill out from left to right during your turn.
...
(Later turns will have empty cells to fill out, but there still won't be any decision-making or die-rolling. For those reasons, AM is not listed in the Phases table on the left of your game sheet.)

### 2. Buy

Before you can make any money <IconsParen ids={["money"]} /> selling widgets, you've got to spend money buying their parts.
...
The <Opportunity /> costs are a table of values, but, for all the phases of your first turn, you'll only use the ones in the leftmost column with the dark outline.

For example, to buy a <A suffix={","} /> you need <OpportunityX value={0} /> and <MoneyX value={1} />; to buy a <B /> you need <OpportunityX value={1} /> and <MoneyX value={2} />; etc. You'll revisit <Opportunity /> costs in the PM phase a little later.
...
1. **Write:**
    * In the Buy column, write the values for the total <Money /> cost, the parts you bought, and the total <Opportunity /> they used.

Notes:

- The value of the <Opportunity /> roll is shared by all players, and everyone fills in their game sheets simultaneously.
- You can buy as many or as few parts as you want and even multiples of the same part, as long as you have the required <Opportunity /> and <Money suffix="." /> (The sum <Opportunity /> must be less than or equal to your roll, and the sum <Money /> must be less than or equal to what you have in the AM column.)
...
1. **Write:**
    * In the Make column, write the values for the parts you used, the widgets you made, and the total <Opportunity /> they used.
...
1. **Write:**
    * In the Sell column, write the values for the <Money /> you made, the widgets you sold, and the total <Opportunity /> they used.
...
1. **Roll:**
    * Roll a die. Its value is the _minimum_ <Opportunity /> you need remaining to upgrade _one_ of the previous three phases' actions. See below for more info.
1. **Write:**
...
Remember how you've only been using the Phase table's first column of <Opportunity /> values with the dark outline? Here's where that can change.

If your remaining <Opportunity /> is greater than or equal to your roll, your research attempt succeeded. Circle the PM column's <Opportunity /> cell to mark its success; otherwise cross it out for failure:
...
Then, if successful, choose _one_ phase action (Buy, Make, or Sell) to spend it on, shade in its outlined column, and then outline the next values to its right.

As an example, here's what a successful research upgrade to Make would look like when done:
...
You spent more <Opportunity /> than you had available at the start of the turn? Now you have to spend the whole next day recuperating. Hope it was worth it.
...
Not a super profitable turn, but it ended with a spare part for later and a successful research upgrade. Could've been a lot worse!
...
{ /* TODO: make sure page/pages matches actual count */}
The rule book's back page is a {RollTableParents} table of {RollTableChildren} randomly generated values.

To use it as a substitute for rolling physical dice, close your eyes and drop a twirling fingertip or non-marking utensil on it. The touched value is your "roll." If the value is obscured, adopt the next readable value being pointed at.

To increase randomness, alternate sections of the table on each use.
...
![Hardware Hustle feedback form QR code](https://quickchart.io/qr?text=https://github.com/oskitone/hardware_hustle&margin=0&size=76&format=svg)
