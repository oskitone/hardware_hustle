...
Then, if successful, choose _one_ phase action (Buy, Make, or Sell) to spend it on, then shade in its outlined column and outline the next values to its right.
...
{ /* TODO: make it singular if there's only one page */}
