...
{/* TODO: try roll+write format here too */}

1. **Tally:** Finish each row's equation into the final column prefixed with **=**.
1. **Resolve <Opportunity />:**
   1. **Roll for research upgrade**. Its value is the <Opportunity /> cost it would take to upgrade _one_ of the previous three phases' actions. See below for more info.
   1. If <Opportunity /> is negative, you suffer **burnout** and lose the entire next turn to rest.
...
Was the <Opportunity /> roll less than or equal to your remaining <Opportunity />? Nice! Your research succeeded. Circle the PM column's <Opportunity /> cell to mark its success; otherwise cross it out for failure:
...
* Its _usage_, however, is different from the other phases. For Buy/Make/Sell, it's a maximum that you want to roll high; for research, it's a minimum that you want to roll low.
