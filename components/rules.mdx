...
1. ### AM
   Take stock and get ready for the day
1. ### Buy
   Spend <Opportunity /> and <Money /> to get Parts (<A /> <B /> <C /> <D />)
1. ### Make
   Spend <Opportunity /> and Parts to get Widgets (<AB /> <ABC /> <ABCD />)
1. ### Sell
   Spend <Opportunity /> and Widgets to get <Money />
1. ### PM
   Use remaining <Opportunity /> to attempt Research, or lose next turn if <Opportunity /> is negative

Buying, Making, and Selling all take <Opportunity />. How much <Opportunity /> is limited by a die roll per Action and a fixed cap per turn.

Let's walk through an example:

## A turn, in detail

Think of a turn as a typical workday: you wake up in the morning, do some work, and then rest or research in the evening. Ideally, the day's actions get you further towards your goal, which, unfortunately, is to make enough money to continue making money.

That day is represented as a spreadsheet. Its rows are grouped into Money, Parts, Widgets, and Opportunity. Its columns are Actions to be filled out from left to right.
...
Remember how you've only been using the first column of <Opportunity /> values with the dark outline? You decide you want to be better at Making, so you shade in the previously used values and outline the whole column of new ones to their right.
...
The player with the most money at the end of the game _without Burning Out in the last turn_ wins! Unused Parts and unsold Widgets count for nothing, only Money.
